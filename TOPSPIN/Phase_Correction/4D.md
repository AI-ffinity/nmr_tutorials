...
parameters are set correctly and the phase angles are zero as explained above, issue `ftnd 0`. 
...
7. **Save and Apply PHC0:**
   Click the **Save-disk nD** icon to copy the values to the 4D spectrum, then click the **Save and Return** icon. Issue `PHC0` and note down the value (here, `-115.0` degrees).
Double-click the processed 4D spectrum and ensure all `PHC0` and 
