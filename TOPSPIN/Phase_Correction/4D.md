...
Specifically for **4D HCNH NOESY**, we need to phase only the **F4(PCH0)** value. There are several ways to do that.
...
there are any. Read the [NUS-reconstruction tutorials](../../Non_Uniform_Sampling/Bruker_NUS/4D.md) for details. Briefly the pulse programs 
...
2. **Initiate NUS-reconstruction with zero phase values:**
   ...
   Follow the instructions at [NUS-reconstruction tutorials](../../Non_Uniform_Sampling/Bruker_NUS/4D.md). Once all 
parameters are set correctly nad the phase angles are zero as explained above, issue `ftnd 0`. 

3. **Extract the 1st FID:**  
   Open the processed 4D spectrum and run `rser eao 1` to extract the 1st FID.

4. **Switch to the Frequency Domain:**  
   Use `ft` to switch to the frequency domain.

5. **Manual Phase Correction:**  
   Enter `.ph` for manual phase correction. Set the pivot line (right mouse click) near the left limit end to modify only `PHC0`, not `PHC1`.  
...
7. **Save and Apply PHC0:**  
   Note down the optimal `PHC0` value (e.g., `-115.0` degrees). Click the **Save-disk nD** icon to copy the values to 
the 4D spectrum, then click the **Return icon**. Double-click the processed 4D spectrum and ensure all `PHC0` and 
`PHC1` values except for F4 (PHC0) are set to `0`.
   ![correct phase parameters](images/CA2_850_Phase_PROCPARS.png)

8. **Repeat NUS Reconstruction:**  
   Use `ftnd 0` to repeat NUS-reconstruction with the correct phase angles.

--------------------------------------------------------------------------
## Solution 2: Phase-Correct the Measured 2D Plane of the 4D Spectrum

**Example Data:** `Raw_Spectra/Carbonic_Anhydrase_2/hCA2-wt_850/6`
...
of the water line (`~4.5 ppm` in `HN`) to avoid filling the spectrum with noise. Then issue the following commands:
...
to the 2D plane. Then click the **Return icon**.
  - Transfer the `PHC0` value to the raw (unprocessed) 4D spectrum, using the `PHC1` value of `0`.

10. **Next Steps:**
  - You are now ready for concomitant NUS-reconstruction and phase correction as described in the [NUS-reconstruction tutorials](../../Non_Uniform_Sampling/Bruker_NUS/4D.md).

------------------------------------------------------------------------------------------------------------------------
## SOLUTION 3: Phase-Correct the F3-F4 projection of the 4D Spectrum.
> **Attention:** This solution is not the most accurate one.

1. **NUS Reconstruction:**
   - Reset **all F4 phase values** to `0` as described in **SOLUTION 1, step 1**.
   - Initiate NUS-reconstruction with zero phase values as described in **SOLUTION 1, step 2**.
...
cannot do zero-filling or truncate the dimensions with `xfb` command. We can only do `abs1` and `abs2` 
...
------------------------------------------------------------------------------------------------------------------------
## SOLUTION 4: Phase-Correct the F3-F4 projection of the 4D Spectrum.
