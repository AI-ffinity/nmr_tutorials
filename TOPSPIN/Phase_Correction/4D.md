...
2. **Extract the 1st FID:**  
  - Open the raw 4D spectrum and run `rser eao 1` to extract the 1st FID.
...
typically use low `NusAMOUNT` values (less than 3%). In addition, the size of the producted *4rrrr* file will be the same
as if you did full processing with `ftnd 0`. If you find the quality insufficient for the next steps, you can 
...
6. **Apply Phase Correction to 4D:**
  ...
  - Execute `pknd 4` (WARNING: very slow as it makes Hilbert Transform on all planes!; may be faster to redo `ftnd 0` or `ftnd 4`)
...
4. **Sum Positive and Negative Projection**  
  - Open `34` and type `wrp 343409` to copy it to a new PROCNO named `343409`.  
...
6. **Apply Phase Correction to 4D:**
  - Copy the `F4(PHC0)` value to the 4D spectrum.
  - Execute `pknd 4` (WARNING: very slow as it does Hilbert Transform on all planes!; may be faster to redo `ftnd 0` or `ftnd 4`)
